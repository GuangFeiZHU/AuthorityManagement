...
def init_permission_session(request,id):
    ...
    # 3 获取用户权限id 和方法
    permission_dict_list = models.Permission2Action2Role.objects.filter(role__in=role_list2).values('permission__url',
    ...
    # 获取权限及菜单列表
    permission_list = list(models.Permission2Action2Role.objects.filter(role__in=role_list2).values(
